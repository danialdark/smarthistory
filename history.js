...
async function saveLastTwoCandleToRedis(lastTwoCandleArray, timeframe, symbolName) {
    await pipeline.set(`${timeframe}-${symbolName.toLowerCase()}`, JSON.stringify(lastTwoCandleArray)).expire(`${timeframe}-${symbolName.toLowerCase()}`, 1200).exec();
...
async function startStreams(symbols) {
    ...
    for (const symbol in symbols) {
        ...
        setTimeout(async () => {
            ...
            await axios.get(`https://historyfx3.chtx.ir/active/${symbol.toUpperCase()}`)
                .then(response => {
